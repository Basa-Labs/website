...
//                            'response' => $res,
    ...
    */
    ...
    static public function sendConfirmationEmail($user_id, $email) {
        ...
        wp_mail( $email, 'Confirm you registration', 'Click on the link to activate your subscription: ' . $activation_link );
    ...
    public static function NFT1155Owners($param) {
        ...
        $nft_moralis_key = 
            (WPSC_helpers::valArrElement($options, "nft_moralis_key") and !empty($options["nft_moralis_key"]))?
                ...
                false;
        if (!$nft_moralis_key) return new WP_REST_Response("Invalid Moralis API key. Are you the system admin? Please setup the Moralis API keys in your WP Smart Contracts settings.", 200 );
        ...
        if ($requests_response->status_code == "200") {
            ...
            return new WP_REST_Response("An error ocurred: " . json_decode($requests_response->body, true)["message"], 200 );    
        }
    ...
    public static function NFT1155MyItems($param) {
        ...
        if (!$address or !$contract or !$network_decimal) return new WP_REST_Response("Invalid parameters.", 200 );
        ...
        if ($response and is_array($response)) {
            ...
            return new WP_REST_Response("An error ocurred", 200 );
        }
    ...
    private static function doCurl($network, $body) {
    ...
    public static function checkDataOnChain($flavor, $block, $network, $txid, $account, $contract, $get_uri = false) {
        ...
        $res = self::doCurl($network, "{\"jsonrpc\":\"2.0\",\"method\":\"eth_getLogs\",\"params\":[{\"blockHash\": \"".$block."\", " . 
                "\"topics\": [" . self::getTopicHash($flavor) . "]}],\"id\":1}");
    ...
    public static function updatePostMetaOnChain($param) {
        ...
        if (!$id) {
            return new WP_REST_Response("Post ID not found", 401);
        }
    ...
    public static function nftInsert($param) {
        ...
        if ($handle===false) {
            return new WP_REST_Response([["message"=>"<span style=\"color: red\">An error ocurred inserting posts</span>"]], 200 );
        }
        ...
        if (!$title) {
            self::SemaphoreSignal($handle);
            return new WP_REST_Response("Title is not valid", 200);
        }
        ...
        if (!$nft_id) {
            ...
            if (($flavor=="yuzu" or $flavor=="ikasumi") and !$supply) {
                self::SemaphoreSignal($handle);
                return new WP_REST_Response("Supply is not valid, please write a positive number", 200);
            }
        }
        ...
        if (is_array($tainted_custom_gals) or is_array($galleries_tainted)) {	
            if (!WPSC_NFTGallery::db_check()) {	
                return new WP_REST_Response("An error ocurred processing galleries. The table: " . WPSC_NFTGallery::db_table_name() . " could not be found on the database. Please check that the database user has \"CREATE TABLE\" permission.", 200 );	
            } else {	
        ...
        if ($the_id) {
            ...
            if (self::validNFTSignature($sign, $collection_id, $the_id)) {
                ...
                return new WP_REST_Response("Not authorized", 401);
            }
        } else {
    ...
    private static function returnMediaData($id, $media, $unique_id, $message, $index, $ipfs=false) {
        ...
        $message = "<span style=\"color: red\">An error (1: $message) ocurred processing media file: " . $media . "</span>";
    ...
    public static function saveMedia($param) {
        ...
        if ($handle===false) {
            return new WP_REST_Response([["message"=>"<span style=\"color: red\">An error (2) ocurred processing media file: " . $media . "</span>"]], 200 );
        }
        ...
        if ($num>0) {
            ...
            return new WP_REST_Response(
                self::returnMediaData($num, $media, $unique_id, "Image ID processed", $index, $ipfs), 
            200);
        } elseif (substr(strtolower($media), 0, 4) == "http") {
            // is an URL
            if ($ret_id = self::upload($media)) {
                return new WP_REST_Response(
                    self::returnMediaData($ret_id, $media, $unique_id, "URL downloaded", $index, $ipfs), 
                200);    
            }
            ...
            if (file_exists($filename)) {
                if ($id = self::upload($filename)) {
                    return new WP_REST_Response(
                        self::returnMediaData($id, $media, $unique_id, "Local file processed", $index, $ipfs),
                    200);    
                }
            }
        }
        if ($unique_id) {
            self::appendTransient($unique_id, "<span style=\"color: red\">An error (3) ocurred processing media file: " . $media . "</span>");
        }

        self::SemaphoreSignal($handle);

        return new WP_REST_Response([["message"=>"<span style=\"color: red\">An error (4) ocurred processing media file: " . $media . "</span>"]], 200 );
    }
    ...
    public static function nftSaveDeploy($param) {
        ...
        if ($sign) {
            if (!WPSC_Queries::validateLazyMinting($nft_id, $sign)) {
                return new WP_REST_Response("Not valid signature found" , 200 );
            }
        ...
        if ($nft_id_check>0) return new WP_REST_Response("Item was already minted" , 200 );
